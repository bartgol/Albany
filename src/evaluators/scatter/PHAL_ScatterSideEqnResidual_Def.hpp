...
gatherSideSetNodeGIDs (const Albany::AbstractDiscretization& disc) {
  // Note: you cannot call this function on a per-worksed basis, since
  //       it is technically possible that this ws does not have any side
  //       on the Eqn sideSet, but still has a node on it:
  //
  //               1-------2-------3
  //                \  A  / \  C  /
  //                 \   /   \   /
  ...
  //       A nor C, then this ws would not notice that 2 is on the sideset.
  //       Therefore, we need to loop over the whole mesh.
  // Note: the scenario above could still happen at the MPI decomp level.
  //       However, it cannot happen for the basal sideset of an extruded
  ...
  //       sideset (in the owned+shared map), then it also has a side on that sideset.

  // Do this only once.
  if (this->ss_nodes_gids.size()>0) {
    return;
  }
