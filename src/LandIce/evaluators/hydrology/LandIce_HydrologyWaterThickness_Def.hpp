                         ...
                         const Teuchos::RCP<Albany::Layouts>& dl)
{
  ...
  Teuchos::RCP<PHX::DataLayout> layout, cell_layout;
  ...
  if (IsStokes) {
    ...
    cell_layout = dl->cell_scalar2_sideset;
  } else {
    layout = nodal ? dl->node_scalar : dl->qp_scalar;
    cell_layout = dl->cell_scalar2;
  }
  ...
  A   = PHX::MDField<const TempScalarT>(p.get<std::string> ("Ice Softness Variable Name"), cell_layout);
