...
void ResponseFactory::createResponseFunction(
  ...
  Teuchos::Array< Teuchos::RCP<AbstractResponseFunction> >& responses) const
{
  ...
  const std::string& name = responseParams.get<std::string>("Name");
  ...
  if (name == "Solution Average") {
  ...
  } else if (name == "Tikhonov Regularization") {
    responses.push_back(
      rcp(new Albany::TikhonovRegularizationResponseFunction(responseParams, comm)));
  } else if (name == "Aggregate Responses" || name == "Sum Responses") {
    Array< RCP<AbstractResponseFunction> > abstract_responses = createResponseFunctions(responseParams);
    Array< RCP<ScalarResponseFunction> > scalar_responses;

    for (auto& resp_i : abstract_responses) {
      TEUCHOS_TEST_FOR_EXCEPTION(
          resp_i->isScalarResponse() != true, std::logic_error,
          "Response function " << resp_i->name() << " is not a scalar response function." <<
          std::endl <<
          "The aggregated response can only aggregate scalar response " << "functions!");
      scalar_responses.push_back(Teuchos::rcp_dynamic_cast<ScalarResponseFunction>(resp_i));
    }
    ...
    createResponseFunction(responseParams.sublist("ResponseParams"), base_responses);
...
ResponseFactory::createResponseFunctions(Teuchos::ParameterList& responseList) const
{
  ...
  for (int i=0; i<num_response_vecs; i++) {
    ...
    Teuchos::ParameterList& response_params = responseList.sublist(sublist_name);
    createResponseFunction(response_params, responses);
  }
